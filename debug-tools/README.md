...
For development and debugging, you may want to build in Debug mode:
...
| rocgdb           | amd-comgr, amd-dbgapi    | bzip2, gmp, mpfr, expat, ncurses, liblzma, zlib, zstd   | System versions of these libraries               |
...
The Python executable must provide a shared `libpython` library (e.g.,
...
cmake -B /therock/output/build -GNinja \
  ...
  -DTHEROCK_TEST_AMDGPU_FAMILIES="gfx942" \
  /therock/src
cmake --build /therock/output/build -t rocgdb
```

```bash
# Use external amd-dbgapi source
cmake -B /therock/output/build -GNinja \
  ...
  -DTHEROCK_TEST_AMDGPU_FAMILIES="gfx942" \
  /therock/src
cmake --build /therock/output/build -t amd-dbgapi
```

A warning message will be displayed during configuration when using external
source directories. Changes to the external source will trigger rebuilds when
using `ninja <component>+build`.

This example demonstrates a typical development workflow: building most of
ROCm in Release mode for performance while keeping debug-tools components in
Debug mode with full debug information.

```bash
# Configure with debug-tools in Debug mode, rest of ROCm in Release
cmake -B /therock/output/build -GNinja \
...
cmake --build /therock/output/build -t amd-dbgapi rocr-debug-agent rocgdb

# After modifying ROCgdb source, rebuild just that component
cmake --build /therock/output/build -t rocgdb+build
```
...
docker run --rm -i -t \
  ...
  ghcr.io/rocm/therock_build_manylinux_x86_64:latest \
  /bin/bash
```
...
For building debug-tools alongside other components:
...
docker run --rm -i -t \
  ...
  --mount type=bind,src=$HOME/therock/output,dst=/therock/output \
  --mount type=bind,src=$HOME/therock,dst=/therock/src \
  ghcr.io/rocm/no_rocm_image_ubuntu24_04_rocgdb:latest \
  /bin/bash
```
